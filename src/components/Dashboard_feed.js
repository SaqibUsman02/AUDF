...
const Dashboard_feed = ({ QueriesData }) => {
  ...
  const Query_Upvote = async (e) => {
    ...
    const res = await fetch("https://df-server.vercel.app/QueryUpvote", {
      method: "POST",
      headers: {
  ...
  const Query_Devote = async (e) => {
    ...
    const res = await fetch("https://df-server.vercel.app/QueryDevote", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
const Query_Vote = async (e,name) => {
  ...
  const res = await fetch("https://df-server.vercel.app/QueryVote", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
    },
...
const CheckQueryVote = async (req, res) => {
  ...
  try {
   const params = new URLSearchParams();
    ...
    const res= await fetch(`https://df-server.vercel.app/VoteData?${params.toString()}`,{
      method:"GET",
      headers: {
    ...
    const data =await res.json();
          // window.alert(data[0]?.Type + " gfgdfgdfgdfgdf "+ data?.Type + "asdsadsa" + data + "6666666666" + data?.PostID +  "  "+ res.UserID);
...
// ************* Comment wale modal ka Code **********************
  ...
  const GetImage = async (req, res) => {
    ...
    try {
      ...
      // const encodedEmail = encodeURIComponent(email);
      params.append("Email", email);     
      const res= await fetch(`https://df-server.vercel.app/OwnPhoto?${params.toString()}`,{
        method:"GET",
        headers: {
            Accept: "application/json",
            "Content-Type": "application/json"
